...
public class Library {
    ...
    public Hashtable<Integer, LibraryCollection> scienceSection; // this can be thought of as bookshelves in different
    ...
    // this.members.remove(member.name.hashCode()); //The hashCode() is from Object
    // class. remove() is a hashtable method.
    // }
    ...
    public void Library(Librarian librarian, Technician technician) {
        this.artSection = new Hashtable<Integer, LibraryCollection>();
        this.scienceSection = new Hashtable<Integer, LibraryCollection>();
        this.newspaperSection = new Hashtable<Integer, LibraryCollection>();
        this.lawSection = new Hashtable<Integer, LibraryCollection>();
    ...
    /*
     * Add librarian: (according to this function, there could only be a unique on
     * duty librarian.)
     * 1. Set the librarian's supervising library as this library
     * 2. Add the new librarian to the employee list
     * 3. Set the librarian as the on duty Libararian if there was no on duty
     * librarian before
     */
    ...
    public void addTechnician(Technician technician) {
        ...
        if (this.ondutyTechnician == null) {
            this.setTechnician(technician);
            return;
        }
    }

    // According to the section and name's hashcode to get the item from the
    // corresponding hash table.
    // The name LibraryCollection is very confusing, is that the object? Or the
    // superclass of the object?
    public LibraryCollection getItem(Section section, String name) {
        switch (section) {
            case ARTS:
                return this.artSection.get(name.hashCode()); // This is a hash table.
            case SCIENCES:
    ...
    // Librarian librarian = new Librarian("Mrs. Scott", "319 Southshore Rd",
    // librarianDOB, "gscottumsystem.edu", librarianSSN, library);
    // library.addLibrarian(librarian);

    // Date studentDOB = new Date(1000);
    // SSN studentSSN = new SSN("552-35-2934");
    // Student student = new Student("Andrew Stormer", "502 Kentucky Blvd",
    // studentDOB, "abspp8@umsystem.edu", studentSSN);

    // Date professorDOB = new Date(1000);
    // SSN professorSSN = new SSN("321-78-4556");
    // Professor professor = new Professor("Dr. Jones", "1312 Providence Rd",
    // professorDOB, "cjones@umsystem.edu", professorSSN);

    // SSN librarianSSN2 = new SSN("321-22-7876");
    // Date librarianDOB2 = new Date(2347);
    // Librarian librarian2 = new Librarian("Mrs. Brown", "515 Wilson Ave",
    // librarianDOB2, "pbrown@umsystem.edu", librarianSSN2, library);

    // SSN technicianSSN = new SSN("888-22-3333");
    // Date technicianDOB = new Date(1245);
    // Technician technician = new Technician("Mr. Scott", "319 Southshore Rd",
    // technicianDOB, "jscott@umsystem.edu", technicianSSN, library);
    ...
    // Newspaper n1 = new Newspaper("Columbia Tribune", 2023, Section.NEWSPAPERS,
    // 5);
